...
**Gate baseline (as of `tp-core-reseller-scope`, branched from `new-setup`):** the suite is 395 tests, `24 failed, 361 passed, 10 errors`, non-zero exit — the 24 failures and 10 errors are the known/accepted baseline, not something to "fix" incidentally; the exact failing/erroring test IDs are the thing to diff, not the totals. The 10 errors are all `gateway/tests/test_views_async.py::event_loop`-fixture collection errors from `pytest-asyncio` 1.x removing that fixture (deliberately pinned this way — see `requirements/ci.txt` comment; migrating those 16 call sites is separate work). Before the original repair the run aborted at `4 passed, 1 error` because `--pdb` on the `--keepdb`/default paths dropped into an interactive debugger on the first failure. `set -e` in `scripts/run-tests.sh` means the script itself always exits 1 once any test fails, even on the accepted baseline — check the pytest summary line and failing test IDs, not the script's own exit code. If the number of collected tests changes, check whether it's real drift vs. a currently-out-of-scope companion change (e.g. new permission tests) landing separately.
