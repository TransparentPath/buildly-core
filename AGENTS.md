...
**Gate baseline (as of this repair, dev @ `42b2fcb`):** the suite is 368 tests, `24 failed, 334 passed, 10 errors`, non-zero exit — this is the known/accepted baseline, not something to "fix" incidentally. The 10 errors are all `gateway/tests/test_views_async.py::event_loop`-fixture collection errors from `pytest-asyncio` 1.x removing that fixture (deliberately pinned this way — see `requirements/ci.txt` comment; migrating those 16 call sites is separate work). Before this repair the run aborted at `4 passed, 1 error` because `--pdb` on the `--keepdb`/default paths dropped into an interactive debugger on the first failure. If the number of collected tests changes, check whether it's real drift vs. a currently-out-of-scope companion change (e.g. new permission tests) landing separately.
