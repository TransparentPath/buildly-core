...
14. **`reset_password_confirm` still uses uid/token (temporarily)**: After the `reset_password_check` rewrite, `CoreUserResetPasswordConfirmSerializer` was deliberately decoupled from `CoreUserResetPasswordCheckSerializer` — it now extends `serializers.Serializer` directly and re-declares `uid`/`token` inline. This keeps `POST /coreuser/reset_password_confirm/` functional on the old uid/token flow until its own follow-up ticket replaces it with the email/code/new_password flow. Do not assume the two actions share validation any more.
